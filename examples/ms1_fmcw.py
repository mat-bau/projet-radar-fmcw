...
from src.signal_processing.range_doppler_map import (
    ...
    generate_range_doppler_map_with_axes,  # Nouvelle fonction qui retourne aussi les axes
    apply_cfar_detector
)
...
def main():
    ...
    try:
        ...
        try:
            # avec zero-padding cette fois ci :')
            ...
            print(f"Range axis: de {range_axis[0]:.2f}m à {range_axis[-1]:.2f}m, {len(range_axis)} points") # ca augmente avec le zero-padding
        ...
        if detections is not None and np.sum(detections) > 0:
            ...
            plt.title(f'Carte Range-Doppler avec détections - Frame {args.frame}')
            plt.grid(True, linestyle='--', alpha=0.5)
            plt.axhline(y=0, color='r', linestyle='-', alpha=0.3)
            plt.tight_layout()
            detect_path = f"{args.output_dir}/range_doppler_detections_frame{args.frame}.pdf"
            ...
            fig2 = plot_range_doppler(
                ...
                range_axis, 
                velocity_axis, 
                title=f"Range-Doppler map (Padding {args.range_padding}x/{args.doppler_padding}x) - Frame {args.frame}",
                dynamic_range=args.dynamic_range,
                save_path=f"{args.output_dir}/range_doppler_map_frame{args.frame}.pdf"
            )
        ...
        fig3 = visualize_3d_range_doppler(
            ...
            range_axis, 
            velocity_axis, 
            title=f"Visualisation 3D Range-Doppler - Frame {args.frame} (Padding {args.range_padding}x/{args.doppler_padding}x)",
            save_path=f"{args.output_dir}/range_doppler_3d_frame{args.frame}.pdf"
        )
        plt.show()
